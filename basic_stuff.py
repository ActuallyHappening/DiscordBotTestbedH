...
class BasicCommands(commands.Cog):
  ...
  async def roulette(self, ctx):
    ...
    await ctx.respond(f"Roulette spun the number {_num}!")
  ...
  async def slot_machine(self, ctx):
    ...
    await ctx.respond(f"Slot machine spun up **{_nums[0]}**, **{_nums[1]}**, **{_nums[2]}**, and **{_nums[3]}**!")
  
  @decorator()
  async def roulette_history(self, ctx):
      await ctx.respond(f"Roulette history: {self._roulette_history}") # TODO: Does not output very pretty :(
  
  @decorator()
  async def slot_machine_history(self, ctx):
      await ctx.respond(f"Slot machine history: {self._slow_machine_history}") # TODO: Does not output very pretty :(
